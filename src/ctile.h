...
static void draw_tile_shadow(int x, int y, int tile_index,int shade_amount)
{
    unsigned char* dst = back_buffer;
    dst = dst + (buffer_width*y) + x;
...
static void draw_tile_rotated_shadow(int x, int y, int tile_index, float angle,int shade_amount)
{
    int tile_index_x = tile_index % TILES_PER_ROW;
    int tile_index_y = tile_index / TILES_PER_ROW;
